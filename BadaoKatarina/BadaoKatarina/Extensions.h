...
{
	return target != nullptr && GEntityList->Player()->IsValidTarget(target, range);
}
...
{
	return SArray<IUnit*>(GEntityList->GetAllMinions(false, true, false)).Where([&](IUnit* i) {return IsValidTarget(i, range) && !IsWard(i); });
}
inline SArray<IUnit*> AllyMinions(float range = 100000)
{
	return SArray<IUnit*>(GEntityList->GetAllMinions(true, false, false)).Where([&](IUnit* i) {return IsValidAllies(i, range) && !IsWard(i); });
}
inline SArray<IUnit*> NeutralMinions(float range = 100000)
{
	return SArray<IUnit*>(GEntityList->GetAllMinions(false, false, true)).Where([&](IUnit* i) {return IsValidTarget(i, range) && !IsWard(i); });
}
